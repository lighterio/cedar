...
Console.defaults = Base.decorate({
  ...
  prefixes: {
    ...
    warn: yellow + (isWindows ? '*' : '\u272D') + ' ',
    error: red + (isWindows ? '\u00D7' : '\u2716') + ' ',
    alert: red + bold + (isWindows ? '*' : '\u2739') + ' '
  },
  ...
  format: function (string, type) {
    ...
    string = string.replace(/^\[(\w+)\] ([^\n]+)/, function (match, name, text) {
      ...
      return text + pad + grey + name + base;
    });
    var before = self.linesBefore;
    string = string.replace(
      /(\n + at )([^\n\/]+?)(:| [\[\(])/g,
      function (match, at, where, then) {
        return grey + '\n   at ' + base + where + grey + then;
      }
    );
    string = string.replace(
      /\n +at ([^:\n]+ )?(\()?(\/[^:]+\/)([^\/:]+):(\d+):(\d+)(\))?/g,
      function (match, where, start, path, file, line, char, end) {
        var shortPath = path;
        for (var i = 0; i < 2; i++) {
          var dir = dirs[i];
